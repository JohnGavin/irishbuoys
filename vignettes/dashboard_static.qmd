...
buoy_data <- readRDS(system.file("extdata", "dashboard_buoy_data.rds", package = "irishbuoys"))
dashboard_stats <- readRDS(system.file("extdata", "dashboard_stats.rds", package = "irishbuoys"))
dashboard_ts <- readRDS(system.file("extdata", "dashboard_timeseries.rds", package = "irishbuoys"))

# Fallback: Load from JSON if RDS not available (development mode)
if (is.null(buoy_data)) {
  data_path <- "data/buoy_data.json"
  buoy_data <- jsonlite::fromJSON(data_path)
...
*Dashboard generated `r Sys.time()` UTC using irishbuoys v`r packageVersion("irishbuoys")`*
