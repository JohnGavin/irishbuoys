...
```{r}
if (has_data) {
  # Faceted plotly by station with high contrast colors
  p <- ggplot(rogue_events, aes(x = time, y = hmax,
    ...
    geom_point(aes(color = rogue_ratio), size = 3, alpha = 0.9) +
    scale_color_viridis_c(name = "Hmax/Hs", option = "inferno", begin = 0.2, end = 0.9) +
  ...
  ggplotly(p, tooltip = "text") |>
    layout(title = paste0("Rogue Wave Events by Station (",
...
if (!is.null(rogue_conditions) && "wind_speed" %in% names(rogue_conditions)) {
  ...
  plot_ly(rc, x = ~wind_speed, y = ~rogue_ratio, color = ~beaufort,
    ...
    layout(
      title = paste0("Rogue Waves by Wind Speed (Beaufort Scale, ",
                     format(min(rc$time), "%Y-%m-%d"), " to ",
                     format(max(rc$time), "%Y-%m-%d"), ")"),
...
if (!is.null(rogue_conditions)) {
  ...
  plot_ly(week_counts, x = ~week, y = ~n, color = ~month_name,
    ...
    layout(
      title = paste0("Rogue Wave Events by Week of Year (",
...
if (!is.null(rogue_conditions) && "time_of_day" %in% names(rogue_conditions)) {
  ...
  plot_ly(tod_data, x = ~time_of_day, y = ~n,
    ...
    layout(
      title = paste0("Rogue Waves by Time of Day (", date_caption, ")"),
