...
This worked example shows exactly how **Signif Wave** (significant wave height) is calculated from raw buoy measurements during one 17.5-minute sampling window.
...
if (!is.null(worked_signif_wave_example)) {
  ...
  cat("**Step 1: Buoy records individual waves during 17.5-min window**\n\n")
  cat("Approximately 100-300 waves are measured (we show 100 here).\n\n")
  ...
  knitr::kable(waves_display,
    caption = "First and last 5 waves (sorted by height, tallest first)",
...
if (!is.null(worked_signif_wave_example)) {
  ...
  cat("**Step 2: Identify the highest 1/3 of waves**\n\n")
  cat(paste0("Total waves: 100\n"))
  cat(paste0("Top 1/3 count: ", example$n_top_third, " waves\n\n"))

  cat("**Step 3: Calculate Signif Wave = mean of top 1/3**\n\n")
  cat(paste0("Signif Wave = mean(top 34 waves) = **", example$signif_wave, " m**\n\n"))

  cat("**Step 4: Max Wave = tallest single wave**\n\n")
  cat(paste0("Max Wave = **", example$max_wave, " m**\n\n"))

  cat("**Step 5: Calculate Ratio**\n\n")
  cat(paste0("Ratio = Max Wave / Signif Wave = ", example$max_wave, " / ", example$signif_wave, " = **", example$ratio, "**\n\n"))

  if (example$ratio > 2.0) {
    cat("::: {.callout-warning}\n")
    cat("**Rogue Wave Detected!** Ratio > 2.0\n")
    cat(":::\n")
  } else {
    cat("::: {.callout-note}\n")
    cat("**Normal Sea State.** Ratio in typical range (1.5-1.9)\n")
    cat(":::\n")
  }
}
```
...
#| echo: false
if (!is.null(worked_signif_wave_example)) {
  cat(worked_signif_wave_example$caption)
}
```
