...
# Helper for DT tables with optimized timestamp display
create_dt <- function(data, caption = "") {
  # Find timestamp columns (typically named 'time')
  ...
  dt <- datatable(
    ...
    options = list(
      ...
      # Set column widths - wider for timestamps
      columnDefs = if (length(time_cols) > 0) {
        list(list(width = '150px', targets = time_cols - 1))  # 0-indexed
      } else NULL
    ),
...
if (is.null(rogue_events)) {
  load_rds <- function(f) {
    # Try relative path (during vignette rendering)
  ...
  # Load rogue wave events (contains weather condition fields)
  rogue_events <- load_rds("rogue_wave_events.rds")
  rogue_conditions <- rogue_events  # Same data with wind_category, season, etc.
  ...
  # Extract gust analysis from summary if available
  if (!is.null(analysis_summary) && "gust_analysis" %in% names(analysis_summary)) {
    gust_analysis <- analysis_summary$gust_analysis
  }

  # Extract annual trends from summary
  if (!is.null(analysis_summary) && "trends" %in% names(analysis_summary)) {
...
if (has_targets) {
  ...
  if (nrow(m2) > 0) {
    m2 %>% arrange(desc(rogue_ratio)) %>%
      select(time, wave_height, hmax, rogue_ratio) %>%
      mutate(
        time = format(time, "%Y-%m-%d %H:%M"),
        across(c(wave_height, hmax), ~round(., 2)),
        rogue_ratio = round(rogue_ratio, 3)
      ) %>%
...
if (has_targets) {
  ...
  if (nrow(m3) > 0) {
    m3 %>% arrange(desc(rogue_ratio)) %>%
      select(time, wave_height, hmax, rogue_ratio) %>%
      mutate(time = format(time, "%Y-%m-%d %H:%M"), across(c(wave_height, hmax), ~round(., 2)), rogue_ratio = round(rogue_ratio, 3)) %>%
