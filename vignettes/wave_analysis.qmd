...
| Max Wave Observed | `r if(has_data) round(max(rogue_events$hmax), 1) else "N/A"` m |
...
**Note: Limited sample size (n=8 years).** The pooled GEV analysis below is illustrative only due to only 8 annual maxima. The GPD per-station approach (Tab 1) is preferred.
