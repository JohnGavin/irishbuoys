...
tar_read_safe <- function(name) {
  # Try targets store in project root
...
if (has_targets && nrow(rogue_events) > 0) {
  rogue_events %>%
    ...
    kable(caption = "All Rogue Wave Events",
          col.names = c("Time", "Station", "Hs (m)", "Hmax (m)", "Hmax/Hs"))
}
```
...
if (has_targets) {
  ...
  if (nrow(m2) > 0) {
    m2 %>% arrange(desc(rogue_ratio)) %>%
      ...
      mutate(across(c(hs, hmax), ~round(., 2)), rogue_ratio = round(rogue_ratio, 3)) %>%
      kable(col.names = c("Time", "Hs (m)", "Hmax (m)", "Hmax/Hs"))
  }
}
```
...
```{r rogue-other}
if (has_targets) {
  for (stn in c("M4", "M5", "M6")) {
    d <- rogue_events[rogue_events$station_id == stn, ]
    cat(paste0("\n**", stn, ":** ", nrow(d), " rogue wave events\n"))
    if (nrow(d) > 0) {
      print(kable(d %>% arrange(desc(rogue_ratio)) %>%
        select(time, hs, hmax, rogue_ratio) %>% head(5) %>%
        mutate(across(c(hs, hmax), ~round(., 2)), rogue_ratio = round(rogue_ratio, 3)),
        col.names = c("Time", "Hs (m)", "Hmax (m)", "Hmax/Hs")))
    }
  }
}
```
...
if (!is.null(rogue_conditions)) {
  rogue_conditions %>%
    head(20) %>%
    select(any_of(c("time", "station_id", "rogue_ratio", "wind_category", "season"))) %>%
    kable(caption = "Sample of rogue wave conditions")
}
```
...
```{r return-wave-table}
if (!is.null(return_levels_wave)) {
  kable(return_levels_wave, digits = 2, caption = "Wave Height Return Levels")
}
```
...
if (!is.null(return_levels_wave) && !is.null(return_levels_hmax)) {
  bind_rows(
    ...
    kable(digits = 1, caption = "Return Levels Summary",
          col.names = c("Variable", "Return Period (yrs)", "Return Level", "Lower CI", "Upper CI"))
}
```
...
if (!is.null(gust_analysis) && "summary" %in% names(gust_analysis)) {
  kable(gust_analysis$summary, digits = 2, caption = "Gust Factor Statistics")
} else {
