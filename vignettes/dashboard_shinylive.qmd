  ...
  tabPanel(
    ...
    fluidPage(
      fluidRow(
        column(6,
          ...
          p("This interactive dashboard displays data from the ",
            ...
            ", operated by the ",
            tags$a(href = "https://www.marine.ie/", "Marine Institute of Ireland", target = "_blank"),
            "."),
          h4("Features"),
          tags$ul(
          ...
          p("The Irish Weather Buoy Network is operated by the ",
            tags$a(href = "https://www.marine.ie/", "Marine Institute of Ireland", target = "_blank"),
            "."),
      ...
      tags$table(
        ...
        tags$tbody(
          tags$tr(
            ...
            tags$td("meters"),
            tags$td("Significant wave height (Signif Wave). Calculated as 4x RMS of wave motion over a 17.5 minute measurement period, then averaged hourly. This is the standard oceanographic measure representing the average height of the highest third of waves.")
          ),
          tags$tr(
            tags$td(tags$b("hmax")),
            tags$td("meters"),
      ...
      tags$ul(
        ...
        tags$li(tags$b("Signif Wave-Max Wave:"), " Strong positive (r > 0.9). Max Wave is derived from same wave record."),
      ...
      p("Data updates: Every 6 hours via GitHub Actions")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$timeseries <- renderPlotly({
    ...
    for (stn in unique(data$station_id)) {
      ...
      p <- p |> add_trace(
        ...
        marker = list(size = 3),
        hoverinfo = "text",
        text = ~paste0("<b>", stn, "</b><br>", format(time, "%Y-%m-%d %H:%M"), "<br>", var_label, ": ", round(get(var), 2))
      )
    }

    p |> layout(
      title = list(text = paste(var_label, "- Last", input$days, "Days"),
                   font = list(size = 14, color = "#e0e0e0")),
      plot_bgcolor = "#1a1a1a", paper_bgcolor = "#1a1a1a",
      xaxis = list(title = "Time (UTC)", gridcolor = "rgba(255,255,255,0.2)",
                   tickfont = list(color = "#cccccc"), titlefont = list(color = "#e0e0e0")),
      yaxis = list(title = var_label, gridcolor = "rgba(255,255,255,0.2)",
                   tickfont = list(color = "#cccccc"), titlefont = list(color = "#e0e0e0")),
      hovermode = "x unified",
      legend = list(orientation = "h", y = -0.15, x = 0.5, xanchor = "center",
                    font = list(color = "#e0e0e0", size = 12),
                    bgcolor = "rgba(40,40,40,0.8)", bordercolor = "#555555", borderwidth = 1),
  ...
  output$scatter_plot <- renderPlotly({
    ...
    plot_ly(data, x = ~get(var_x), y = ~get(var_y),
            ...
            text = ~paste0(
              "<b>", station_id, "</b><br>",
              x_label, ": ", round(get(var_x), 2), "<br>",
              y_label, ": ", round(get(var_y), 2)
            )) |>
      layout(
        title = list(text = paste(y_label, "vs", x_label, "- Last", input$scatter_days, "Days"),
                     font = list(size = 14, color = "#e0e0e0")),
        plot_bgcolor = "#1a1a1a", paper_bgcolor = "#1a1a1a",
        xaxis = list(title = x_label, gridcolor = "rgba(255,255,255,0.2)",
                     tickfont = list(color = "#cccccc"), titlefont = list(color = "#e0e0e0")),
        yaxis = list(title = y_label, gridcolor = "rgba(255,255,255,0.2)",
                     tickfont = list(color = "#cccccc"), titlefont = list(color = "#e0e0e0")),
        legend = list(orientation = "h", y = -0.15, x = 0.5, xanchor = "center",
                      font = list(color = "#e0e0e0", size = 12),
                      bgcolor = "rgba(40,40,40,0.8)", bordercolor = "#555555", borderwidth = 1),
  ...
  output$last_updated <- renderText({
    ...
    # Data validation: real ERDDAP data has Wind-Wave corr ~0.3, Wave-Max Wave ~0.97
    ...
    paste0(
      ...
      "Signif Wave-Max Wave corr: ", ifelse(is.na(cor_wave_hmax), "N/A", cor_wave_hmax), " (expect >0.9 for real data)\n",
      "Max Signif in dataset: ", round(max(data$wave_height, na.rm = TRUE), 1), "m (historical max ~15m Signif Wave)\n",
      "Max Wave in dataset: ", round(max(data$hmax, na.rm = TRUE), 1), "m (historical max ~23m individual wave)\n",
      "Max Wave/Signif Wave ratio: ", round(max(data$hmax/data$wave_height, na.rm = TRUE), 2), " (>2.0 = rogue wave)\n",
      "Max wind_speed: ", round(max(data$wind_speed, na.rm = TRUE), 1), " m/s\n",
      "Max gust factor: ", round(max(data$gust/data$wind_speed, na.rm = TRUE), 2), " (typical ~1.3)"
    )
  })
