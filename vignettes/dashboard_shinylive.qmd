...
server <- function(input, output, session) {
  ...
  # Load data from Parquet file (167KB compressed vs 3.2MB JSON)
  # In Shinylive, the file is bundled via resources: in YAML
  buoy_data <- reactive({
    data <- NULL

    # Try Parquet first (compressed, efficient)
    parquet_paths <- c(
      "data/buoy_data.parquet",
      "./data/buoy_data.parquet"
    )
    ...
    for (path in parquet_paths) {
      tryCatch({
        if (file.exists(path) && requireNamespace("nanoparquet", quietly = TRUE)) {
          data <- nanoparquet::read_parquet(path)
          data$time <- as.POSIXct(data$time, format = "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")
          message("Loaded ", nrow(data), " records from Parquet: ", path)
          break
        }
      }, error = function(e) {
        message("Parquet load failed from ", path, ": ", e$message)
      })
    }
      ...
      for (path in json_paths) {
        tryCatch({
          if (file.exists(path)) {
            ...
            data$time <- as.POSIXct(data$time, format = "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")
            message("Loaded ", nrow(data), " records from JSON: ", path)
            break
          }
        }, error = function(e) {
          message("JSON load failed from ", path, ": ", e$message)
        })
      }
    }

    # Final fallback - generate sample data with REALISTIC correlations
    # This should never happen if Parquet is bundled correctly
    if (is.null(data) || nrow(data) < 100) {
      message("WARNING: Using fallback sample data - real data failed to load")
      message("Check browser console (F12) for data loading errors")
      n_days <- 90
      n_per_day <- 24
      n_stations <- 5
      n_total <- n_days * n_per_day * n_stations

      # Generate correlated data (not random!)
      set.seed(42)
      base_wind <- rnorm(n_total, 15, 8)
      base_wind <- pmax(2, pmin(50, base_wind))

      data <- data.frame(
        time = rep(Sys.time() - (1:(n_days * n_per_day)) * 3600, n_stations),
        station_id = rep(c("M2", "M3", "M4", "M5", "M6"), each = n_days * n_per_day),
        wave_height = 0.5 + 0.15 * base_wind + rnorm(n_total, 0, 1),
        wind_speed = base_wind,
        gust = base_wind * 1.3 + rnorm(n_total, 0, 2),
        air_temperature = rnorm(n_total, 10, 3),
        sea_temperature = rnorm(n_total, 11, 2),
        atmospheric_pressure = rnorm(n_total, 1013, 15),
      ...
      # hmax correlated with wave_height
      data$hmax <- data$wave_height * 1.6 + rnorm(n_total, 0, 0.5)
      data$wave_period <- 4 + 0.3 * data$wave_height + rnorm(n_total, 0, 1)

      # Ensure positive values
      data$wave_height <- pmax(0.3, data$wave_height)
      data$hmax <- pmax(0.5, data$hmax)
      data$wave_period <- pmax(3, data$wave_period)
    }
