...
server <- function(input, output, session) {
  ...
  output$qc_summary <- renderTable({
    ...
    if (!"qc_flag" %in% names(data)) {
      return(data.frame(Note = "QC flag data not available"))
    }
    ...
    qc_counts[, c("QC_Flag", "Description", "Count", "Percentage")]
  })

  output$qc_by_station <- renderTable({
    data <- buoy_data()
    if (!"qc_flag" %in% names(data)) {
      return(data.frame(Note = "QC flag data not available"))
    }
